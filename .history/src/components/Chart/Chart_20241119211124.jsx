...
const useChartData = (timeframe, barCount) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Slice the data to get the last N bars
      const slicedData = formattedData.slice(-barCount);
      setData(slicedData);
      
      // Set the current index to the last bar of the full dataset
...
const useChartInstance = (containerRef, timeframe) => {
  ...
  const backtestSeriesRef = useRef(null);
  ...
  useEffect(() => {
    ...
    // Add backtest series for potential trading signals
    backtestSeriesRef.current = chartRef.current.addLineSeries({
      color: 'rgba(255, 255, 0, 0.7)', // Yellow with transparency
      lineWidth: 2,
    });
  ...
  return { chartRef, candleSeriesRef, backtestSeriesRef };
};
...
const TradingChart = () => {
  ...
  useEffect(() => {
    if (data && candleSeriesRef.current) {
      ...
      if (data.length > 0) {
        ...
        // Simple backtest example: Moving average crossover
        // if (data.length > 20) {
        //   const shortMA = calculateMovingAverage(data, 10);
        //   const longMA = calculateMovingAverage(data, 20);
          
        //   backtestSeriesRef.current.setData(
        //     shortMA.map((value, index) => ({
  ...
  // Backtest function: Simple Moving Average
