...
const TradingChart = () => {
  ...
  const handleTimeframeChange = (newTimeframe) => {
    const epochStart = new Date('2000-01-01T00:00:00Z').getTime() / 1000; // Epoch in seconds

    if (!data || !fullDataset) return;
    ...
    const currentStartTime = data[0]?.time; // Start time of the current range
    if (!currentStartTime) return;
  
    const newInterval = intervalMinutes[newTimeframe];
    const currentEpochTime = currentStartTime - epochStart; // Time in seconds since epoch
    const currentIndexInNewTimeframe = Math.floor(
      (currentEpochTime / 60) / newInterval
    );
  
    const startIndexForNewTimeframe = Math.max(0, currentIndexInNewTimeframe);
    setTimeframe(newTimeframe);
    setStartIndex(startIndexForNewTimeframe); // Sync with new time frame
  };
