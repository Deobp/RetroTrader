...
const useChartData = (timeframe, barCount, historyPoint) => {
  ...
  const lastHistoryPointRef = useRef(historyPoint);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Calculate start index based on history point or use default
      let startPoint;
      if (timeframe !== lastTimeframeRef.current) {
        // If timeframe changed, maintain the relative position
        const relativePosition = lastHistoryPointRef.current 
          ? lastHistoryPointRef.current / formattedData.length 
          : 1;
        startPoint = Math.floor(formattedData.length * relativePosition) - barCount;
      } else {
        startPoint = historyPoint ?? Math.max(0, formattedData.length - barCount);
      }

      // Ensure startPoint is within valid range
      startPoint = Math.max(0, Math.min(startPoint, formattedData.length - barCount));
      ...
      lastHistoryPointRef.current = startPoint;
  ...
  }, [timeframe, barCount, historyPoint]);
  ...
  const addNextCandle = useCallback(() => {
    ...
    if (lastIndex < fullDataset.length - 1) {
      ...
      setData(prevData => {
        ...
        lastHistoryPointRef.current = lastIndex - barCount + 2;
        return newData;
      });
    }
  }, [fullDataset, data, barCount]);
  ...
  return { 
    ...
    fullDataset,
    addNextCandle,
    currentHistoryPoint: lastHistoryPointRef.current
  };
};
...
const TradingChart = ({ timeframe }) => {
  ...
  const { 
    ...
    addNextCandle,
    currentHistoryPoint
  } = useChartData(timeframe, barCount, historyPoint);
  ...
  const handleRandomDateNavigation = () => {
    if (fullDataset) {
      const randomIndex = Math.floor(Math.random() * (fullDataset.length - barCount));
      setHistoryPoint(randomIndex);
    }
  };
