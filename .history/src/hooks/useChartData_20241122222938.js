...
export const useChartData = (timeframe, barCount) => {
  ...
  const normalizeTimestamp = (timestamp) => {
    ...
    // Assuming your current timestamps are offset from a different reference point
    const baseOffset = 4655520; // Your first timestamp
    const targetOffset = Math.floor(Date.UTC(1970, 0, 1) / 1000); // Unix epoch start
    const correctedTimestamp = timestamp + (targetOffset - baseOffset);
